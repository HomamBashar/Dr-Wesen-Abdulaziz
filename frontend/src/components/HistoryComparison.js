...
const EyeColumn = ({ title, data, compareAgainst }) => (
  <div className="flex-1 min-w-0">
    <h4 className="text-xs font-bold text-slate-500 dark:text-ink-300 uppercase tracking-wider mb-2 text-center">{title}</h4>
    <div className="rounded-lg border border-slate-200 dark:border-ink-700 overflow-hidden">
      {EYE_ROWS.map((row, i) => {
        ...
        return (
          ...
          >
            <span className="text-slate-400 dark:text-ink-300 text-xs">{row.label}</span>
    ...
    <div className="mt-4">
      {!open ? (
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-5">
          ...
          {loading ? (
            <div className="text-center py-10 text-sm text-slate-400 dark:text-ink-300">جاري التحميل...</div>
          ) : history.length === 0 ? (
            <div className="text-center py-10 text-sm text-slate-400 dark:text-ink-300">
            ...
            <>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="text-center">
                  <p className="text-xs font-bold text-slate-400 dark:text-ink-300 uppercase">زيارة سابقة</p>
              ...
              {(selected?.diagnosis || currentData?.diagnosis) && (
                <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-100 dark:border-ink-700">
                  <div>
                    <p className="text-xs font-bold text-slate-400 dark:text-ink-300 uppercase mb-1">التشخيص (سابق)</p>
                    <p className="text-sm text-slate-600 dark:text-ink-200">{selected?.diagnosis || '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-400 dark:text-ink-300 uppercase mb-1">التشخيص (حالي)</p>
                    <p className="text-sm text-slate-600 dark:text-ink-200">{currentData?.diagnosis || '—'}</p>
                  </div>
                </div>
              )}

              <div className="flex items-center gap-4 mt-4 pt-3 border-t border-slate-100 dark:border-ink-700 text-xs text-slate-400 dark:text-ink-300">
