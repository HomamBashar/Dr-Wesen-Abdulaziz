...
const SecretaryPage = () => {
  ...
  const fetchPatients = useCallback(async (search = "") => {
    try {
      ...
      setPatients(data.items.filter(p => p.status !== 'completed' && p.status !== 'scheduled'));
    } catch (e) { /* silent */ }
  }, []);

  const fetchAppointments = useCallback(async () => {
    try {
      const { data } = await apiClient.get('/patients', { params: { status: 'scheduled', limit: 100 } });
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-ink-300">واجهة السكرتير</p>
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-ink-900 rounded-2xl border border-slate-200 dark:border-ink-700 p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-ink-300">{today}</p>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-ink-900 rounded-2xl border border-slate-200 dark:border-ink-700 p-6 min-h-full">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-ink-300 mt-1">
              ...
              <p className="text-xs text-slate-400 dark:text-ink-300 mb-3">اضغط على مريض لفتح ملفه ومساعدة الدكتورة</p>
              <div className="space-y-2 max-h-[500px] overflow-y-auto">
                {patients.length === 0 ? (
                  <div className="text-center py-12 text-slate-400 dark:text-ink-300">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-ink-300 mt-0.5">
        ...
        <section>
          <div className="bg-white dark:bg-ink-900 rounded-2xl border border-slate-200 dark:border-ink-700 p-6">
            ...
            {appointments.length === 0 ? (
              <div className="text-center py-10 text-slate-400 dark:text-ink-300">
                ...
                {appointments.map((a) => (
                  ...
                  >
                    <div>
                      ...
                      {a.appointment_note && (
                        <p className="text-xs text-slate-400 dark:text-ink-300 mt-1">{a.appointment_note}</p>
                      )}
                    </div>
